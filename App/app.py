...
def load_data():
    ...
    candidate_paths = [
        base_dir / "restaurant_ranking.csv",
        base_dir / "data" / "restaurant_ranking.csv",
        base_dir / "final.csv",
        base_dir / "data" / "final.csv",
        Path.cwd() / "restaurant_ranking.csv",
        Path.cwd() / "data" / "restaurant_ranking.csv",
        Path("/mnt/data/restaurant_ranking.csv"),
    ]

    csv_path = next((path for path in candidate_paths if path.exists()), None)
    if csv_path is None:
        searched = "\n".join(str(path) for path in candidate_paths)
        raise FileNotFoundError(f"Could not find restaurant_ranking.csv. Looked in:\n{searched}")
    ...
    if "lng" in df.columns:
        ...
        raise ValueError("The CSV must contain 'lat' and 'lng' or 'lon'.")

    df = df.dropna(subset=["lat", "lon"]).copy()

    rename_map = {
        "title": "restaurant_name",
        "safety_pct": "safety_percent",
    ...
    def normalize_price_tier(value):
        if pd.isna(value):
            return None
    ...
    df["price_tier"] = df["price"].apply(normalize_price_tier)
    df["price_display"] = df["price_tier"].fillna("")
...
def load_reviews():
    ...
    candidate_paths = [
        base_dir / "clean_data.csv",
        base_dir / "data" / "clean_data.csv",
    ]

    csv_path = next((p for p in candidate_paths if p.exists()), None)
    if csv_path is None:
        return pd.DataFrame(columns=["title", "text", "title_norm", "text_norm"])
    ...
    text_col = None
    for candidate in ["text", "review", "review_text", "comment", "comments", "content", "body", "reviewText"]:
    ...
    temp = temp[["title", text_col]].copy()
    temp.columns = ["title", "text"]

    temp["title"] = temp["title"].fillna("").astype(str)
    temp["text"] = temp["text"].fillna("").astype(str)

    temp = temp[temp["text"].str.strip() != ""].copy()

    temp["title_norm"] = temp["title"].apply(normalize_text)
    temp["text_norm"] = temp["text"].apply(normalize_text)

    return temp.drop_duplicates(subset=["title", "text"])
...
def load_images():
    ...
    candidate_paths = [
        base_dir / "imagenes.csv",
        base_dir / "data" / "imagenes.csv",
    ]

    img_path = next((path for path in candidate_paths if path.exists()), None)
    if img_path is None:
        raise FileNotFoundError("No se encontró imagenes.csv")

    img_df = pd.read_csv(img_path).copy()

    if "title" not in img_df.columns:
        raise ValueError("imagenes.csv debe tener una columna 'title'")
...
def get_reviews_for_restaurant(restaurant_name, reviews_df, max_reviews=12):
    ...
    restaurant_reviews = reviews_df[reviews_df["title_norm"] == name_norm].copy()
    ...
    gluten_pattern = r"gluten|gluten free|glutenfree|gluten-free|gf|celiac|coeliac"

    restaurant_reviews = restaurant_reviews[
        restaurant_reviews["text_norm"].str.contains(gluten_pattern, na=False, regex=True)
    ].copy()
...
def _collect_distinct_image_urls(matched_rows, max_images=8, visual_dup_threshold=8):
    possible_cols = ["imageUrl"]
    for i in range(20):
        possible_cols.append(f"imageUrls/{i}")
        possible_cols.append(f"images/{i}/imageUrl")
    ...
    def image_signature(url):
        ...
        stem = re.sub(r"(copy|edited|original|preview|thumbnail|thumb|small|medium|large|banner)$", "", stem)
        stem = re.sub(r"[-_]?\d{1,4}$", "", stem)
        stem = re.sub(r"[^a-z0-9]", "", stem)

        tail = "/".join(parts[-2:]) if len(parts) >= 2 else (parts[-1] if parts else "")
        tail = re.sub(r"\.[a-z0-9]{2,5}$", "", tail)
        tail = re.sub(r"[^a-z0-9/]", "", tail)

        return stem or tail or lowered

    raw_candidates = []
    for _, img_row in matched_rows.iterrows():
        for col in possible_cols:
            if col in img_row and pd.notnull(img_row[col]):
                raw_url = str(img_row[col]).strip()
                if raw_url:
                    raw_candidates.append(raw_url)

    # Mantener el orden original, pero sin URLs exactas repetidas.
    ordered_candidates = []
    seen_candidate_urls = set()
    for raw_url in raw_candidates:
        normalized_url = normalize_image_url(raw_url).lower()
        if not normalized_url or normalized_url in seen_candidate_urls:
            continue
        seen_candidate_urls.add(normalized_url)
        ordered_candidates.append(raw_url)
    ...
    for raw_url in ordered_candidates:
        ...
        signature = image_signature(raw_url)
        ...
        img_hash = get_image_hash(raw_url)
        visually_repeated = False
        if img_hash:
            for existing_hash in selected_hashes:
                if hamming_distance(img_hash, existing_hash) <= visual_dup_threshold:
                    visually_repeated = True
                    break

        if visually_repeated:
            continue
        ...
        distinct_urls.append(raw_url)

        if len(distinct_urls) >= max_images:
            break

    return distinct_urls[:max_images]
...
if not st.session_state.show_map and not st.session_state.map_transition:
    ...
    def set_cover_background():
        ...
        candidate_paths = [
            base_dir / "skyline.png",
            base_dir / "data" / "skyline.png",
        ]
...
# RESTAURANT PINS
# -------------------------
visible_rows = filtered_df.head(80).copy()
