...
@st.cache_data
def load_images():
    """Load images.csv from the correct project path only."""
    path = PROJECT_ROOT / "data" / "raw" / "images.csv"

    if not path.exists():
        st.error(f"No se encontró images.csv en: {path}")
    ...
    if title_col is None:
        st.error("images.csv no tiene columna title")
        return pd.DataFrame(columns=["title", "title_norm"])
    ...
    for col in get_image_columns(img_df):
        img_df[col] = (
            img_df[col]
            .fillna("")
            .astype(str)
        ...
        with col1:
            ...
            if restaurant_images:
                ...
                for img_url in restaurant_images:
                    safe_url = html.escape(str(img_url), quote=True)
                    collage_html += f'''<div class="photo-tile"><img src="{safe_url}" loading="lazy" onerror="this.parentElement.style.display=\'none\';"></div>'''
